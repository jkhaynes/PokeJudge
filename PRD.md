...
| 6 | RAG | Ruling generation now conditioned on retrieved chunks + accumulated game state, **and** the Milestone 2 clarification logic is rebuilt as the retrieve → assess → clarify → re-retrieve loop from section 11, replacing pretrained-knowledge-based materiality with retrieval-grounded materiality. A first-pass Source Support classification (Strong/Partial/Insufficient) is introduced here as an output of ruling generation. |
