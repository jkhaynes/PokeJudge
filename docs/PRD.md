...
| 3+ | *(Optional, conditional)* Community ruling compendium ingestion | Extend the Milestone 3 ingestion pipeline with a second source type — card-interaction rulings from a third-party community database (e.g., `compendium.pokegym.net`) — via a second `IDocumentExtractor`-style implementation behind the same `IngestedDocument`/`IngestedSection` model, with per-ruling citation metadata instead of document+section. **Contingent on obtaining explicit permission from the site operator first.** The site's `robots.txt` explicitly disallows AI crawlers (including Claude's) and signals `ai-train=no`; do not scrape or bulk-copy its content, by automation or by hand, without permission. Entirely optional — skip permanently if permission isn't granted; not a dependency for any later-numbered milestone. |
...
- Whether the optional Milestone 3+ (community ruling compendium ingestion) ever happens at all — contingent entirely on the site operator granting explicit permission; not assumed, not scheduled, and skipped permanently if permission isn't given.
...
**Milestone 3+ (Optional) — Community ruling compendium ingestion**
Understand extending an ingestion pipeline to a second, structurally different source type (community-authored web rulings vs. an official PDF) behind the same output model — and, separately, understand that "can we technically scrape this" and "should we" are different questions. `compendium.pokegym.net`'s `robots.txt` explicitly disallows AI crawlers (Claude's included) and signals `ai-train=no`. This milestone only proceeds if the site operator grants explicit permission; otherwise it stays permanently skipped. It exists in the roadmap as a placeholder for that possibility, not as planned work.
